      subroutine s_rqfn(n,p,a,y,rhs,d,beta,eps,tau,wn,wp,aa,nit,info)
...
23002 continue
      ...
      call s_fna(n,p,a,y,rhs,d,beta,eps,wn(1,1),wn(1,2),wp(1,1),wn(1,3),
     ...
     &wp(1,3), wp(1,4),aa,nit,info)
      ...
      subroutine s_fna(n,p,a,c,b,d,beta,eps,x,s,y,z,w,dx,ds,dy,dz,dw,
     ...
     &p,p)
      ...
      nit(3)=n
      pp=p*p
      call s_dgemv('N',p,n,one,a,p,c,1,zero,y,1)
      call s_stepy(n,p,a,d,y,aa,info)
...
23008 continue
      ada(i,i)=one
23006 continue
      call s_dtrtrs('U','T','N',p,p,aa,p,ada,p,info)
      call dcopy(pp,ada,1,aa,1)
      call dcopy(n,c,1,s,1)
      call s_dgemv('T',p,n,mone,a,p,y,1,one,s,1)
      do 23010 i=1,n
      d(i)=one
      z(i)=dmax1(s(i),zero)
      w(i)=dmax1(-s(i),zero)
...
23012 if(.not.(rdg .gt. eps))goto 23013
      ...
      dx(i)=d(i)*ds(i)
23014 continue
      call s_dgemv('N',p,n,one,a,p,dx,1,zero,dy,1)
      call dcopy(p,dy,1,rhs,1)
      call s_stepy(n,p,a,d,dy,ada,info)
      if(.not.(info .ne. 0))goto 23016
      return
23016 continue
      call s_dgemv('T',p,n,one,a,p,dy,1,mone,ds,1)
      deltap=big
      deltad=big
      do 23018 i=1,n
      dx(i)=d(i)*ds(i)
...
23030 continue
      ...
      call s_dgemv('N',p,n,one,a,p,dz,1,one,dy,1)
      call s_dpotrs('U',p,1,ada,p,dy,p,info)
      call daxpy(p,mone,dy,1,rhs,1)
      call s_dgemv('T',p,n,one,a,p,rhs,1,zero,dw,1)
      deltap=big
      deltad=big
      do 23032 i=1,n
...
23013 continue
      ...
      subroutine s_stepy(n,p,a,d,b,ada,info)
...
23040 continue
      do 23044 i=1,n
      call s_dsyr('U',p,d(i),a(1,i),1,ada,p)
23044 continue
      call s_dposv('U',p,1,ada,p,b,p,info)
      return
      end
